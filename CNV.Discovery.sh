...
usage() {
  ...
  echo "required options:"
    ...
    echo "-e|--excludedFileList    the samples in this list will be excluede from CNVR detection, their genotyping are reported based on the CNVR boundaries defined by other samples. This option is applicable to the outgroup individual"
    
    echo "-f|--frequency           frequency of gain/loss individuals when define a candidate CNV window"
    echo "                         [recommend 0.1]"
    
    echo "-h|--homozygous          number of homozygous gain/loss individuals when define a candidate CNV window"
    echo "                         [recommend 3]"
...
perl $CNVcaller/bin/2.1.CNVDiscoveryMerge.pl $refdb $normalizedFileList $excludedFileList $primaryCNVR -k1 $windowsize -k2 $frequency -k3 $homozygous -k6 $pearsonCorrelation
