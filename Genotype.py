...
def genotype(cnvays):
    ...
    for cnvay in cnvays:
        ...
        dpgmm = BayesianGaussianMixture(n_components=n_com,n_init=n_init,max_iter=10000,
        ...
        if len(np.unique(finalline)) > 1:
            ...
            sc = np.nan
            chs = np.nan
        llh = dpgmm.score(cnv) # Log likelihood of the Gaussian mixture given X
...
#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT''')
        ...
        for row, cns in zip(df.values, cnvays):
            ...
            silhouette_score = row[-3]
            calinski_harabaz_score = row[-2]
